...
// it. This can be useful to communicate completion to channel's receives.

func main() {

  // In this example we'll use jobs channel to communicate work to be
  // done from main() goroutine to a worker goroutine.
  ...
  // j, more := <-jobs. In this special 2-value form of recieve, the more
  ...
  // This sends 2 jobs to worker over the jobs channel, then close it.
  for j := 1; j <= 3; j++ {
    jobs <- j
