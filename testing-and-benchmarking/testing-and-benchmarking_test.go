// Unit testing is an important part of wirting pricipled Go 
// programs. The testing package provides the tools we need
// to write unit tests and the go test command runs tests.

// For the sake of demonstration, this code is in pacakge main,
// but if could be any package. Testing code typically lives 
...
// something like intuitls.go, and the test file for it would then
...
func TestIntMinBasic(t *testing.T) {
  ...
  if ans != -2 {
    ...
    // the test. t.Fatal* will report tet failures and stop the
    // test immediately.
...
// are listed in a table and a single loop walks them 
...
func TestIntMinTableDriven(t *testing.T) {
  ...
  for _, tt := range tests {
    ...
    // These are show seperately when executing go test -v.
...
// executes each benchmark function several times,
// increasing b.N on each run until it collects a precise
// measurement.
func BenchmarkIntMin(b *testing.B) {
  // Typically the benchmark runs a function we're benchmarking
  // in a loop b.N times.
  for i := 0; i < b.N; i++ {
    IntMin(1,2)
  }
